...
class ZebrunnerApiClient {
  ...
  async sendLogs(test, level = 'INFO', messages) {
    if (this.testsMap.get(test.uniqueId)) {
      ...
      var headers = await this.getHeadersWithAuth(jsonHeaders);
      return this.httpClient.callPost(url, logsAggr, headers, true).then(() => {
  ...
  async parseResultsAndSendVideo(suiteTestsDurationsMap) {
    ...
    if (suiteTestsDurationsMap) {
      ...
      this.logger.info(path.basename(__filename), `cant split and send video. there are no tests in suite "${message.suite.title}"`)
    } else {
