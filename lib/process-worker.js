...
process.on('message', (message) => {
  ...
  switch (event) {
    ...
    case EVENT_SUITE_BEGIN:
      ...
      break;
    //тут этот ивент дополнен и теперь к мисседжу добавлен  message.suiteTestsDurations
    case EVENT_SUITE_END:
      if (message.suite.title) {
        this.logger.info(path.basename(__filename), 'suite finished. name: ' + message.suite.title)
      }
      //там содержится вся инфа с завершённого сьюта по тестам(название, длительность, статус, айди, к какому спеку принадлежит и тд)
      //всю эту инфу добавляю в мапу по завершении каждого сьюта
    ...
    case workerEvents.PARENT_PROCESS_END:
      //splitting and sending video report logic
      var promises = []
      promises.push(zebrunnerApiClient.parseResultsAndSendVideo(suiteTestsDurationsMap))
